...
def estimate_pose(camera_matrix, dist_coeffs, completed_img_dict):
    camera_offset = -0.4
    ...
    for target_num in completed_img_dict.keys():
        ...
        true_width = target_dimensions[target_num - 1][0]

        pixel_width = box[2][0]
        pixel_center = box[0][0]

        # Correct for lens distortion
        distorted_points = np.array([[pixel_center, 0]], dtype=np.float32)
        undistorted_points = cv2.undistortPoints(distorted_points, camera_matrix, dist_coeffs)
        pixel_center = undistorted_points[0][0]

        distance = true_width / pixel_width * focal_length
...
if __name__ == "__main__":
    ...
    target_est = estimate_pose(camera_matrix,dist_coeffs, completed_img_dict)
    print("target_est: ")
    print(target_est)
