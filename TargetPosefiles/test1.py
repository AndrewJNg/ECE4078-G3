...
def estimate_pose(camera_matrix, completed_img_dict):
    ...
    # New
    focal_length_mm = 4.15 # mm #TODO Find out proper value
    f_x, f_y = camera_matrix[0][0], camera_matrix[1][1]
    m_x, m_y = f_x/focal_length_mm, f_y/focal_length_mm
    m = (m_x + m_y)/2 # Pixels per mm
    print(camera_matrix)
    ...
    for target_num in completed_img_dict.keys():
        ...
        image_height = 480
        image_width = 640
        print(f"Pixel Height: {pixel_height}pix")
        x = 676/3264 * image_height
        pixel_height_mm = pixel_height/ x
        distance = true_height * (focal_length_mm / pixel_height_mm)/1000
        print(f"Distance: {distance}m")
        
        x_shift = image_width / 2 - pixel_center
        theta = np.arctan(x_shift / (focal_length_mm/1000))
