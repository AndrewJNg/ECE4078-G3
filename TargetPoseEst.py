...
def merge_estimations(target_map):
    ...
    if len(redapple_est) > num_per_target:
        sum_x = 0
        sum_y = 0
        for i in redapple_est:
            sum_x = sum_x + i[0]
            sum_y = sum_y + i[1]
        redapple_est = [sum_x/len(redapple_est), sum_y/len(redapple_est)]
    if len(greenapple_est) > num_per_target:
        sum_x = 0
        sum_y = 0
        for i in greenapple_est:
            sum_x = sum_x + i[0]
            sum_y = sum_y + i[1]
        greenapple_est = [sum_x/len(greenapple_est), sum_y/len(greenapple_est)]
    if len(orange_est) > num_per_target:
        ...
        for i in orange_est:
            sum_x = sum_x + i[0]
            sum_y = sum_y + i[1]
        orange_est = [sum_x/len(orange_est), sum_y/len(orange_est)]
    if len(mango_est) > num_per_target:
        ...
        for i in mango_est:
            sum_x = sum_x + i[0]
            sum_y = sum_y + i[1]
        mango_est = [sum_x/len(mango_est), sum_y/len(mango_est)]
    if len(capsicum_est) > num_per_target:
        sum_x = 0
        sum_y = 0
        for i in capsicum_est:
            sum_x = sum_x + i[0]
            sum_y = sum_y + i[1]
        capsicum_est = [sum_x/len(capsicum_est), sum_y/len(capsicum_est)]
